...
import { useKelasList, useSantriList, useMurojaahList, useCreateMurojaah } from '@/lib/hooks/useApi';
import { useQueryClient } from '@tanstack/react-query';
import {
  RefreshCw, Search, Filter, CheckCircle2, AlertCircle, X, Save
} from 'lucide-react';

export default function ManajemenMurojaah() {
  ...
  const [editingMap, setEditingMap] = useState<Record<string, {
    juz: string; surah: string; ayat: string; nilai: string; status_murojaah: string; predikat: string;
  }>>({});

  const { data: kelasData } = useKelasList();
  const { data: santriData } = useSantriList();
  const { data: murojaahData } = useMurojaahList(selectedKelasId ? { kelas_id: selectedKelasId } : undefined);
  ...
  const murojaahRecords = useMemo(() => murojaahData?.data || [], [murojaahData]);
  ...
  const mergedList = useMemo(() => {
    return santriInKelas.map((s: any) => {
      const rec = murojaahRecords.find((r: any) => r.santuario_id === s.id);
      return { santri: s, record: rec || null };
    });
  }, [santriInKelas, murojaahRecords]);
  ...
  const initEdit = (santri: any, record: any) => {
    setEditingMap(prev => ({
      ...
      [santri.id]: {
        ...
        status_murojaah: record?.status_murojaah || '',
  ...
  const handleSave = async (santriId: string) => {
    ...
    try {
      await createMurojaah.mutateAsync({
        ...
        status_murojaah: data.status_murojaah,
        predikat: data.predikat,
      });
      triggerToast('Data Murojaah berhasil disimpan!');
      qc.invalidateQueries({ queryKey: ['murojaah'] });
    } catch {
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-tosca-50/40 border-b border-tosca-100">
                    ...
                    <th className="px-4 py-4 text-[10px] font-bold text-tosca-700 uppercase tracking-widest">Surah</th>
                    ...
                    <th className="px-4 py-4 text-[10px] font-bold text-tosca-700 uppercase tracking-widest">Status Murojaah</th>
                  ...
                  {!selectedKelasId ? (
                    <tr>
                      <td colSpan={12} className="py-12 text-center text-slate-400 font-bold">
                        <RefreshCw size={40} className="mx-auto text-tosca-200 mb-3 stroke-[1.5]" />
                        <p className="text-sm">Pilih kelas terlebih dahulu untuk melihat data Murojaah.</p>
                      </td>
                    </tr>
                        ...
                        <tr key={s.id} className="hover:bg-tosca-50/20 transition-colors group">
                          ...
                          {isEditing ? (
                            <>
                              ...
                              <td className="px-2 py-3">
                                <input value={editVal.surah} onChange={(e) => handleEdit(s.id, 'surah', e.target.value)}
                                  className="w-28 px-2 py-1.5 border border-tosca-200 rounded-lg text-xs font-semibold text-center focus:ring-2 focus:ring-tosca-500" placeholder="Al-Baqarah" />
                              </td>
                              <td className="px-2 py-3">
                                <input value={editVal.ayat} onChange={(e) => handleEdit(s.id, 'ayat', e.target.value)}
                                  className="w-20 px-2 py-1.5 border border-tosca-200 rounded-lg text-xs font-semibold text-center focus:ring-2 focus:ring-tosca-500" placeholder="1-10" />
                              </td>
                              <td className="px-2 py-3">
                              ...
                              <td className="px-2 py-3">
                                <select value={editVal.status_murojaah} onChange={(e) => handleEdit(s.id, 'status_murojaah', e.target.value)}
                            ...
                            <>
                              ...
                              <td className="px-4 py-3 text-sm font-bold text-slate-800">{r?.surah || <span className="text-slate-300">-</span>}</td>
                              ...
                              <td className="px-4 py-3">
                                {r?.status_murojaah ? (
                                  <span className={`px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider inline-block ${
                                    r.status_murojaah === 'Lancar' ? 'bg-green-100 text-green-700 border border-green-200' :
                                    r.status_murojaah === 'Cukup' ? 'bg-tosca-100 text-tosca-700 border border-tosca-200' :
                                    r.status_murojaah === 'Kurang' ? 'bg-amber-100 text-amber-700 border border-amber-200' :
                                    'bg-red-100 text-red-700 border border-red-200'
                                  }`}>{r.status_murojaah}</span>
