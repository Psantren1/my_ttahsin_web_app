...
import { useKelasList, useSantriList, useTahfidzList, useCreateTahfidz } from '@/lib/hooks/useApi';
import { useQueryClient } from '@tanstack/react-query';
import {
  BookMarked, Search, Filter, CheckCircle2, AlertCircle, Loader2, X, Save
} from 'lucide-react';
...
export default function ManajemenTahfidz() {
  ...
  const [editingMap, setEditingMap] = useState<Record<string, {
    juz: string; surat: string; ayat: string; hafalan_baru: string; nilai: string; status_setoran: string; predikat: string;
  }>>({});

  const { data: kelasData } = useKelasList();
  const { data: santriData } = useSantriList();
  const { data: tahfidzData } = useTahfidzList(selectedKelasId ? { kelas_id: selectedKelasId } : undefined);
  ...
  const tahfidzRecords = useMemo(() => tahfidzData?.data || [], [tahfidzData]);
  ...
  const mergedList = useMemo(() => {
    return santriInKelas.map((s: any) => {
      const rec = tahfidzRecords.find((r: any) => r.santuario_id === s.id);
      return { santri: s, record: rec || null };
    });
  }, [santriInKelas, tahfidzRecords]);
  ...
  const initEdit = (santri: any, record: any) => {
    setEditingMap(prev => ({
      ...prev,
      [santri.id]: {
        juz: record?.juz?.toString() || '',
        surat: record?.surat || '',
        ayat: record?.ayat || '',
        hafalan_baru: record?.hafalan_baru || '',
        nilai: record?.nilai?.toString() || '',
        status_setoran: record?.status_setoran || '',
  ...
  const handleSave = async (santriId: string) => {
    ...
    try {
      await createTahfidz.mutateAsync({
        ...
        juz: data.juz ? parseInt(data.juz) : null,
        surat: data.surat,
        ayat: data.ayat,
        hafalan_baru: data.hafalan_baru,
        nilai: data.nilai ? parseInt(data.nilai) : null,
        status_setoran: data.status_setoran,
        predikat: data.predikat,
      });
      triggerToast('Data Tahfidz berhasil disimpan!');
      qc.invalidateQueries({ queryKey: ['tahfidz'] });
    } catch {
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-tosca-50/40 border-b border-tosca-100">
                    ...
                    <th className="px-4 py-4 text-[10px] font-bold text-tosca-700 uppercase tracking-widest">Juz</th>
                    <th className="px-4 py-4 text-[10px] font-bold text-tosca-700 uppercase tracking-widest">Surat</th>
                    <th className="px-4 py-4 text-[10px] font-bold text-tosca-700 uppercase tracking-widest">Ayat</th>
                    <th className="px-4 py-4 text-[10px] font-bold text-tosca-700 uppercase tracking-widest">Hafalan Baru</th>
                    <th className="px-4 py-4 text-[10px] font-bold text-tosca-700 uppercase tracking-widest">Nilai</th>
                    <th className="px-4 py-4 text-[10px] font-bold text-tosca-700 uppercase tracking-widest">Status Setoran</th>
                  ...
                  {!selectedKelasId ? (
                    <tr>
                      <td colSpan={13} className="py-12 text-center text-slate-400 font-bold">
                        <BookMarked size={40} className="mx-auto text-tosca-200 mb-3 stroke-[1.5]" />
                        <p className="text-sm">Pilih kelas terlebih dahulu untuk melihat data Tahfidz.</p>
                      </td>
                    </tr>
                          ...
                          {isEditing ? (
                            <>
                              ...
                              <td className="px-2 py-3">
                                <input value={editVal.juz} onChange={(e) => handleEdit(s.id, 'juz', e.target.value)}
                              ...
                              <td className="px-2 py-3">
                                <input value={editVal.surat} onChange={(e) => handleEdit(s.id, 'surat', e.target.value)}
                                  className="w-28 px-2 py-1.5 border border-tosca-200 rounded-lg text-xs font-semibold text-center focus:ring-2 focus:ring-tosca-500" placeholder="Al-Fatihah" />
                              </td>
                              <td className="px-2 py-3">
                                <input value={editVal.ayat} onChange={(e) => handleEdit(s.id, 'ayat', e.target.value)}
                                  className="w-20 px-2 py-1.5 border border-tosca-200 rounded-lg text-xs font-semibold text-center focus:ring-2 focus:ring-tosca-500" placeholder="1-7" />
                              </td>
                              <td className="px-2 py-3">
                                <input value={editVal.hafalan_baru} onChange={(e) => handleEdit(s.id, 'hafalan_baru', e.target.value)}
                                  className="w-28 px-2 py-1.5 border border-tosca-200 rounded-lg text-xs font-semibold text-center focus:ring-2 focus:ring-tosca-500" placeholder="Al-Fatihah 1-7" />
                              </td>
                              <td className="px-2 py-3">
                              ...
                              <td className="px-2 py-3">
                                <select value={editVal.status_setoran} onChange={(e) => handleEdit(s.id, 'status_setoran', e.target.value)}
                            ...
                            <>
                              ...
                              <td className="px-4 py-3 text-xs font-semibold text-tosca-600">{s.kelas_nama}</td>
                              <td className="px-4 py-3 text-sm font-bold text-slate-800">{r?.juz ?? <span className="text-slate-300">-</span>}</td>
                              <td className="px-4 py-3 text-sm font-bold text-slate-800">{r?.surat || <span className="text-slate-300">-</span>}</td>
                              <td className="px-4 py-3 text-sm font-bold text-slate-800">{r?.ayat || <span className="text-slate-300">-</span>}</td>
                              <td className="px-4 py-3 text-sm font-bold text-slate-800">{r?.hafalan_baru || <span className="text-slate-300">-</span>}</td>
                              ...
                              <td className="px-4 py-3">
                                {r?.status_setoran ? (
                                  <span className={`px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider inline-block ${
                                    r.status_setoran === 'Lancar' ? 'bg-green-100 text-green-700 border border-green-200' :
                                    r.status_setoran === 'Cukup' ? 'bg-tosca-100 text-tosca-700 border border-tosca-200' :
                                    r.status_setoran === 'Kurang' ? 'bg-amber-100 text-amber-700 border border-amber-200' :
                                    'bg-red-100 text-red-700 border border-red-200'
                                  }`}>{r.status_setoran}</span>
