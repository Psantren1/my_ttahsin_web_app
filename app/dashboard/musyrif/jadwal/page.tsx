...
import { getJadwalByMusyrif } from '@/lib/services/jadwal.service';
...
export default async function JadwalMusyrifPage() {
  const session = await getSession();
  if (!session || session.role !== 'MUSYRIF') redirect('/login');

  const rawJadwal = await getJadwalByMusyrif(session.userId);
  ...
  return (
    <>
      <div className="bg-indigo-50 rounded-2xl border border-indigo-100 p-4 flex items-center gap-3">
        ...
        <p className="text-sm text-indigo-700 font-medium">Jadwal sinkron dengan database. Diperbarui secara real-time dari admin.</p>
      </div>
